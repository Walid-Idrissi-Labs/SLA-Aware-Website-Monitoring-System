...
/**
 * Edge-tracking mesh-gradient border glow. Tuned to the HUD theme:
 * translucent frosted surface, ~12px radius, faint white hairline, and a
 * status-colored glow that lights up as the pointer nears an edge (hover-only
 * unless `animated` runs its one-time reveal sweep).
 *
 * `glowColor` is an "H S L" triple (e.g. "152 60 52"); `colors` feeds the
 * mesh fill. Pass a metric's status hex to key the glow to its state.
 */
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          background: [
            `linear-gradient(${coverColor} 0 100%) padding-box`,
